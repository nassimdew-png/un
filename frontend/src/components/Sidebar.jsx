...
export default function Sidebar({ tenant, user, isMobileOpen, onCloseMobile }) {
  ...
  const menuItems = [
    ...
    { label: t('nav.analytics', 'محلل البيانات والـ BI'), path: '/analytics/ai-analyst', icon: BarChart3, badge: 'AI ⚡', badgeColor: 'bg-blue-500/20 text-blue-300 border border-blue-500/30', show: true },
    { label: t('nav.document_processor', 'معالج الفواتير والوثائق'), path: '/finance/document-processor', icon: Receipt, badge: 'OCR ✨', badgeColor: 'bg-amber-500/20 text-amber-300 border border-amber-500/30', show: isClinicAdmin || isSuperAdmin },
  ...
  const adminItems = [
    ...
    { label: '🤖 موظف الاستقبال الذكي (AI Bot)', path: '/settings/ai-receptionist', icon: Bot, badge: 'NEW', badgeColor: 'bg-indigo-500/20 text-indigo-300 border border-indigo-500/30', show: isClinicAdmin || isSuperAdmin },
  ];
  ...
  const superAdminItems = [
    ...
    { label: '📊 تحليل بيانات المنصة (BI)', path: '/analytics/ai-analyst', icon: BarChart3, show: isSuperAdmin },
