...
ARCHIVE_NAME = "patch_impersonation_audit_fix.tar.gz"
...
def main():
    ...
    scp_cmd = [SCP_BIN, "-i", SSH_KEY, "-o", "StrictHostKeyChecking=no", "-o", "ConnectTimeout=15", archive_path, f"{HOST}:{REMOTE_BASE}/"]
...
php -l backend/app/Services/AuditLogger.php
...
"""
    ssh_cmd = [SSH_BIN, "-i", SSH_KEY, "-o", "StrictHostKeyChecking=no", "-o", "ConnectTimeout=15", HOST, remote_script]
    ...
    if "=== DEPLOYMENT COMPLETE ===" in res.stdout:
        print("\nSUCCESS: Impersonation Audit Fix deployed and verified on VPS!")
    else:
        print("\nWARNING: Deployment verification string not found in output.")
